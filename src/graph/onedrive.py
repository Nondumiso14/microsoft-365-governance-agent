...
class OneDriveScanner:
    ...
    # AFTER — matches the tool name exactly
    def scan_user_onedrive(self) -> list[dict]:
        """
        Scans the user's OneDrive for risky permissions.

        Called by: tool_executor._scan_user_onedrive()
        Schema name: scan_user_onedrive (tools.py)
        """
        findings: list[dict] = []
        ...
        for file in files:
            ...
            for perm in permissions:
                risk = self._classify_permission(file_name, item_id, perm)
                if risk:
